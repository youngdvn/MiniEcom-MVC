...
<div class="container-fluid">
    <div class="d-flex flex-wrap align-items-center justify-content-between gap-2 mb-3">
        <div>
            ...
            <small class="text-secondary">Quản trị nhóm sản phẩm hiển thị trên client</small>
        </div>
        <a href="{{ route('admin.cate.create') }}" class="btn btn-primary">+ Thêm danh mục</a>
    </div>
    ...
    <div class="admin-section mb-3">
        <form method="GET" action="{{ route('admin.cate.index') }}">
            <div class="row g-2">
                <div class="col-md-4">
                ...
                <div class="col-md-3 d-flex gap-2">
                    ...
                    <a href="{{ route('admin.cate.index') }}" class="btn btn-outline-secondary">Đặt lại</a>
                </div>
            </div>
        </form>
    </div>
    ...
    <div class="admin-table-wrap">
        <div class="table-responsive">
            <table class="table admin-table align-middle mb-0">
                ...
                <tbody>
                    @forelse($list as $cate)
                        <tr>
                            <td class="text-center">{{ $cate->cateid }}</td>
                            <td class="text-center">
                                <img src="{{ asset('storage/categories/' . ($cate->thumbnail ?? 'default.png')) }}" width="72" class="img-thumbnail" alt="thumb">
                            </td>
                            ...
                            <td class="text-center">
                                <a href="{{ route('product.category', ['slug' => $cate->slug]) }}" target="_blank" class="btn btn-outline-info btn-sm">Xem</a>
                                <a href="{{ route('admin.cate.edit', ['id' => $cate->cateid]) }}" class="btn btn-outline-primary btn-sm">Sửa</a>
                                <form action="{{ route('admin.cate.del', ['id' => $cate->cateid]) }}" method="POST" class="d-inline">
                                    @csrf
                                    @method('DELETE')
                    ...
                    @empty
                        <tr><td colspan="7" class="text-center py-4 text-muted">Không có dữ liệu danh mục.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
    ...
    <div class="mt-3 d-flex flex-wrap align-items-center justify-content-between gap-2">
        <small class="text-muted">Hiển thị {{ $list->firstItem() ?? 0 }} - {{ $list->lastItem() ?? 0 }} / {{ $list->total() }} danh mục</small>
